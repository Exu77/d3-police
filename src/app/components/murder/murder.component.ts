import { Component, OnInit, Inject } from '@angular/core';
//import * as murderData from 'src/assets/murder.guardian.json';
import * as d3 from 'd3';
import { GuardiansServiceService } from '../../services/guardians-service.service';
import {
  IMurderGraph,
  IMurderCaseGuardian,
  ILink,
  INode
} from '../../services/models';
import { SimulationNodeDatum } from 'd3';

@Component({
  selector: 'app-murder',
  templateUrl: './murder.component.html',
  styleUrls: ['./murder.component.scss']
})
export class MurderComponent implements OnInit {
  public dataPost: any;
  public dataGuard: any;
  public murderNodes: any[];
  public simulation;
  public width = 1000;
  public height = 400;

  // https://www.theguardian.com/us-news/ng-interactive/2015/jun/01/the-counted-police-killings-us-database
  // https://data.census.gov/cedsci/table?q=race&hidePreview=false&table=C02003&tid=ACSDT1Y2018.C02003&lastDisplayedRow=18
  // https://github.com/washingtonpost/data-police-shootings

  constructor(private guardiansService: GuardiansServiceService) {
    const forceX = d3.forceX(this.width / 4).strength(0.05);
    const forceY = d3.forceY(this.height / 4).strength(0.05);

    this.simulation = d3
      .forceSimulation()
      .force(
        'link',
        d3.forceLink().id((d: any) => {
          return d.id;
        })
      )
      .force('charge', d3.forceManyBody())
      .force('center', d3.forceCenter(this.width / 2, this.height / 2))
      .force('x', forceX)
      .force('y', forceY);
  }

  ngOnInit() {
    this.drawMurders();
  }

  private drawMurders() {
    const murderNodes = this.guardiansService.getData();
    const allLinks = this.guardiansService.getLinks();

    const xticked = this.ticked;
    const allNodes = this.guardiansService.getAllNodes();

    console.log('aaa', murderNodes, allNodes);

    // const myimage = svg.append('image')
    // .attr('xlink:href', 'http://lorempixel.com/200/200/')
    // .attr('width', 200)
    // .attr('height', 200)

    const context: any = d3
      .select('div.appMurderComp')
      .append('svg')
      .attr('width', this.width)
      .attr('height', this.height)
      .attr('fill', 'magenta');

    const link = context
      .append('g')
      .attr('class', 'link')
      .selectAll('line')
      .data(allLinks)
      .enter()
      .append('line')
      .attr('class', 'armedLink')
      .attr('fill', 'red')
      .attr('stroke', 'green')
      .attr('stroke-width', (d: ILink) => {
        return Math.sqrt(d.value);
      });

    const genderCircleDef = d3
      .select('svg')
      .append('defs')
      .append('g')
      .attr('id', 'genderCircle')
      .append('circle')
      .attr('r', 30);

    const raceCircleDef = d3
      .select('svg')
      .append('defs')
      .append('g')
      .attr('id', 'raceCircle')
      .append('circle')
      .attr('r', 12);

    const murderCircleDef = d3
      .select('svg')
      .append('defs')
      .append('g')
      .attr('id', 'murderCircle')
      .append('circle')
      .attr('r', 3);

    this.createPistolDef();
    this.createWaterPistolDef();
    this.createKnifeDef();
    this.createVehicleDef();
    this.createFistDef();
    this.createQuestionMarkDef();
    this.createMagicWandDef();
    this.createMaleDef();
    this.createFemaleDef();

    const node = context
      .append('g')
      .attr('class', 'node murderNode')
      .selectAll('circle')
      .data(allNodes, d => d.id)
      .enter()
      .append('g')
      .append('use')
      .attr('xlink:href', d => d.svgId)
      .attr('fill', d => d.color)
      .call(
        d3
          .drag()
          .on('start', d => this.dragstarted(d, this.simulation))
          .on('drag', d => this.dragged(d, this.simulation))
          .on('end', d => this.dragended(d, this.simulation))
      );

    const xxx = context.selectAll('svg .node circle');
    this.simulation.nodes(allNodes).on('tick', bla => {
      return this.ticked(link, node);
    });
    // simulation.nodes(nodeArmed).on('tick', () => this.ticked(link, nodeArmed));
    this.simulation.force('link').links(allLinks);

    node.append('title').text(function(d: IMurderCaseGuardian) {
      return d.name;
    });
  }

  private dragstarted(d: any, simulation: any) {
    if (!d3.event.active) {
      this.simulation.alphaTarget(0.3).restart();
    }
    d.fx = d.x;
    d.fy = d.y;
  }

  private dragged(d: any, simulation: any) {
    d.fx = d3.event.x;
    d.fy = d3.event.y;
  }

  private dragended(d: any, simulation: any) {
    if (!d3.event.active) {
      console.log('dragended', this.simulation, d);
      this.simulation.alphaTarget(0);
    }
    d.fx = null;
    d.fy = null;
  }

  private ticked(link, node) {
    link
      .attr('x1', function(d: any) {
        return d.source.x;
      })
      .attr('y1', function(d: any) {
        return d.source.y;
      })
      .attr('x2', function(d: any) {
        return d.target.x;
      })
      .attr('y2', function(d: any) {
        return d.target.y;
      });

    node.attr('transform', d => `translate(${d.x},${d.y})`);
  }

  private createKnifeDef() {
    const knifeDef = d3
      .select('svg')
      .append('defs')
      .append('g')
      .attr('id', 'knife');
    knifeDef.append('circle').attr('r', '20');
    knifeDef
      .append('path')
      .attr('style', 'fill:#030104;')
      .attr('transform', 'translate(-12,-12)')
      .attr(
        'd',
        `M9.043,13.852l2.582,1.728c-0.622,4.832-6.143,8.545-6.143,8.545l-2.034-1.898L9.043,13.852z
        M20.677,0l-0.039,0.584c-0.674,10.17-5.928,15.016-5.982,15.065l-0.143,0.129l-4.956-3.31l6.442-8.994
        c0.361-0.566,1.615-2.461,2.299-2.597c0.18-0.035,0.343,0.028,0.445,0.174c0.017,0.021,0.059,0.082,0.25,0.044
        c0.418-0.083,1.032-0.545,1.234-0.716L20.677,0z M19.887,1.234c-0.252,0.154-0.538,0.298-0.798,0.349
        c-0.402,0.081-0.619-0.081-0.723-0.209c-0.355,0.13-1.277,1.315-1.955,2.379l-6.153,8.587l4.202,2.806
      c0.488-0.494,2.19-2.374,3.617-5.707L18.05,9.398c0,0,0.938-3.564-0.174-3.85c-1.112-0.285-0.955-3.313,0.328-2.598
      C19.203,3.508,19.708,1.952,19.887,1.234z M8.595,13.225l4.081,2.724l0.514-0.768l-4.082-2.725L8.595,13.225z M18.66,10.554
      c0,0-0.471,1.026-0.375,1.507c0.096,0.479,0.465,0.813,0.822,0.741c0.359-0.072,0.572-0.52,0.477-0.999
      C19.489,11.322,18.66,10.554,18.66,10.554z M19.288,9.152c0,0-0.16,0.353-0.129,0.517c0.033,0.165,0.158,0.278,0.282,0.254
      c0.123-0.024,0.196-0.177,0.164-0.342S19.288,9.152,19.288,9.152z`
      );
  }

  private createWaterPistolDef() {
    const waterPistolDef = d3
      .select('svg')
      .append('defs')
      .append('g')
      .attr('id', 'waterpistol')
      .attr('transform', 'scale(0.05 0.05)');

    waterPistolDef.append('circle').attr('r', '500');

    waterPistolDef
      .append('path')
      .attr('style', 'fill:#030104;')
      .attr('transform', 'translate(-160,-220)')
      .attr(
        'd',
        `M378.433,222.609v44.522c0,12.295-9.966,22.261-22.261,22.261h-11.13v5.565
      c0,12.295-9.966,22.261-22.261,22.261H189.216c-12.295,0-22.261,9.966-22.261,22.261l-11.13,111.304
      c0,18.442-14.949,33.391-33.391,33.391h-33.39c-18.441,0-33.391-14.949-33.391-33.391V339.478c0-12.295-9.966-22.261-22.261-22.261
      h-11.13C9.966,317.217,0,307.251,0,294.957v-55.652c0-36.883,29.9-66.783,66.783-66.783h5.565C32.392,172.522,0,140.13,0,100.174
      l0,0c0-39.956,32.391-72.348,72.348-72.348h189.216c39.956,0,72.348,32.391,72.348,72.348l0,0c0,39.956-32.391,72.348-72.348,72.348
      h61.217c12.295,0,22.261,9.966,22.261,22.261v5.565h11.13C368.466,200.348,378.433,210.314,378.433,222.609z`
      );
  }

  private createPistolDef() {
    const pistolDef = d3
      .select('svg')
      .append('defs')
      .append('g')
      .attr('id', 'pistol')
      .attr('transform', 'scale(0.05 0.05)');

    pistolDef.append('circle').attr('r', '500');

    pistolDef
      .append('path')
      .attr('style', 'fill:#030104;')
      .attr('transform', 'translate(-280,-250)')
      .attr(
        'd',
        `M460.173,217.1c6.299-3.8,8.298-10.416,8.433-19.04h0.5c3.128,0,5.669-2.539,5.669-5.661v-89.541
      c0-3.133-2.541-5.658-5.669-5.658h-30.689c-5.94-4.575-12.649-8.198-19.905-10.649l1.284-2.911c1.265-2.87-0.032-6.205-2.895-7.464
      l-5.188-2.298c-2.856-1.262-6.2,0.033-7.462,2.903l-2.802,6.326c-1.665-0.118-3.339-0.197-5.028-0.197H66.546v-3.305
      c0-3.133-2.54-5.661-5.666-5.661H33.979c-3.127,0-5.658,2.528-5.658,5.661v3.305H16.519l4.286,24.55H5.664
      c-3.129,0-5.664,2.526-5.664,5.659v69.844c0,3.133,2.535,5.672,5.664,5.672h29.319l5.603,32.091H226.94
      c2.774,11.297,10.625,32.428,32.181,47.77c12.773,9.084,31.716,13.697,56.323,13.697c8.04,0,15.327-0.515,20.969-1.067
      l37.236,151.334c0.243,3.107,2.961,5.45,6.074,5.208l126.119-0.262c3.118-0.239,5.457-2.961,5.22-6.085L460.173,217.1z
        M315.449,268.013c-21.297,0-37.932-3.824-48.115-11.069c-15.815-11.262-22.728-26.594-25.662-36.235h60.525
      c-4.329,11.078-10.637,22.607-18.397,25.521c-2.198,0.818-3.305,3.272-2.482,5.463c0.635,1.71,2.255,2.758,3.973,2.758
      c0.496,0,1-0.08,1.486-0.265c11.906-4.465,19.907-20.564,24.559-33.478h10.222l11.447,46.49
      C328.064,267.647,322.013,268.013,315.449,268.013z`
      );
  }

  private createVehicleDef() {
    const vehicleDef = d3
      .select('svg')
      .append('defs')
      .append('g')
      .attr('id', 'vehicle')
      .attr('transform', 'scale(0.8 0.8)');

    vehicleDef.append('circle').attr('r', '30');

    vehicleDef
      .append('path')
      .attr('style', 'fill:#030104;')
      .attr('transform', 'translate(-18,-20)')
      .attr(
        'd',
        `M38.831,14.26c-0.191-0.233-0.476-0.369-0.775-0.369h-3.801c-0.938-2.474-2.16-4.898-3.549-5.813    c-4.805-3.161-17.55-3.161-22.355,0c-1.39,0.916-2.607,3.343-3.55,5.813H1c-0.302,0-0.586,0.136-0.775,0.369    c-0.19,0.232-0.266,0.539-0.204,0.834l0.563,2.728c0.096,0.465,0.506,0.797,0.979,0.797h1.126    c-1.087,1.254-1.614,2.833-1.621,4.413c-0.007,1.952,0.734,3.716,2.089,4.964c0.015,0.013,0.03,0.022,0.044,0.035v3.817    c0,0.827,0.672,1.5,1.5,1.5h3.506c0.828,0,1.5-0.673,1.5-1.5v-1.534h19.641v1.534c0,0.827,0.672,1.5,1.5,1.5h3.506    c0.826,0,1.5-0.673,1.5-1.5v-3.742c1.438-1.317,2.125-3.129,2.134-4.938c0.006-1.634-0.545-3.271-1.696-4.551h1.201    c0.475,0,0.885-0.332,0.979-0.798l0.564-2.727C39.094,14.799,39.021,14.494,38.831,14.26z M9.998,10.583    c3.83-2.521,15.229-2.521,19.057,0c0.744,0.488,1.701,2.461,2.578,4.877H7.422C8.297,13.045,9.254,11.073,9.998,10.583z     M5.512,23.408c0-1.63,1.322-2.95,2.951-2.95c1.631,0,2.951,1.32,2.951,2.95s-1.32,2.951-2.951,2.951    C6.834,26.359,5.512,25.038,5.512,23.408z M30.631,26.359c-1.629,0-2.951-1.321-2.951-2.951s1.322-2.95,2.951-2.95    c1.631,0,2.951,1.32,2.951,2.95S32.26,26.359,30.631,26.359z`
      );
  }

  private createFistDef() {
    const fistDef = d3
      .select('svg')
      .append('defs')
      .append('g')
      .attr('id', 'fist')
      .attr('transform', 'scale(0.08 0.08)');

    fistDef.append('circle').attr('r', '300');

    fistDef
      .append('path')
      .attr('style', 'fill:#030104;')
      .attr('transform', 'translate(-160,-200)')
      .attr(
        'd',
        `m 88.154402,424.11278 c -10.43501,-0.3656 -14.02282,-0.91173 -14.52116,-2.21035 -0.74149,-1.93226 0.48807,-12.36736 5.06485,-42.98414 1.71514,-11.47368 4.20876,-28.86825 5.5413,-38.65463 1.33259,-9.78639 3.28105,-22.91019 4.33005,-29.164 1.04896,-6.25384 1.51959,-12.05202 1.04584,-12.8849 -2.3436,-4.12017 -13.9498,-21.46747 -31.80108,-47.53175 C 23.434422,200.4859 4.8816417,172.8848 2.7964417,168.83291 c -3.20282004,-6.22374 -2.45846004,-8.88762 5.67327,-20.30413 4.2047103,-5.90311 10.1298103,-14.34661 13.1669503,-18.76336 15.25243,-22.18054 42.70606,-60.849077 45.00438,-63.388663 1.10386,-1.219764 2.97953,-2.217769 4.16821,-2.217769 1.96531,0 75.205758,31.834119 88.246878,38.356692 3.48855,1.74479 5.91808,3.94389 6.63744,6.00776 1.33808,3.83911 -1.28759,24.53581 -3.49577,27.55571 -1.6827,2.30123 -6.51074,3.30081 -29.27711,6.06155 -21.81236,2.64506 -24.48873,1.82758 -37.294988,-11.39127 -4.86331,-5.01999 -9.36869,-8.80199 -10.01188,-8.40444 -1.39235,0.8605 1.26998,11.84338 3.83617,15.82548 5.03882,7.81901 3.09639,11.4653 -6.97978,13.1023 -7.68789,1.24897 -10.60259,2.88712 -8.61905,4.8442 0.64117,0.63263 9.81888,4.41598 20.39494,8.40744 14.222108,5.36751 20.043558,8.18344 22.356368,10.8143 1.71993,1.95641 3.14466,3.89202 3.16597,4.30137 0.0214,0.40933 2.73742,5.37922 6.03573,11.04417 50.78948,87.23221 49.64597,85.14736 48.72271,88.82589 -0.46435,1.85002 -3.59298,4.93387 -8.13415,8.01769 -4.05871,2.75626 -7.03565,5.56778 -6.61528,6.24785 0.42026,0.68007 3.60949,1.06415 7.08705,0.85351 l 6.32292,-0.383 -0.29931,11.65776 c -0.16472,6.41178 -1.69116,28.50015 -3.39217,49.08526 -5.06691,61.31825 -5.46916,65.43748 -6.56317,67.2076 -0.99157,1.60434 -48.74777,2.82796 -74.778368,1.91597 z m 89.020768,-0.17895 c -4.87585,-0.561 -6.13572,-3.92522 -5.29108,-14.12827 2.48777,-30.05217 4.46899,-52.40149 5.02268,-56.65704 0.35122,-2.69969 1.71157,-16.46 3.02301,-30.57852 1.31136,-14.11846 2.93962,-26.22531 3.6184,-26.90405 0.82731,-0.82736 3.0739,-0.81307 6.81638,0.0434 3.0702,0.70258 5.85372,0.83821 6.18551,0.30142 0.33172,-0.5368 -2.62803,-6.39939 -6.57725,-13.028 -3.9493,-6.62858 -11.20483,-18.95462 -16.12347,-27.39114 -4.91863,-8.43654 -10.18506,-17.27188 -11.70327,-19.63413 -1.51813,-2.36223 -9.80667,-16.342 -18.41897,-31.06616 -10.84314,-18.53832 -15.26605,-27.16382 -14.38209,-28.04777 0.88399,-0.88398 4.61397,-0.83265 12.12885,0.16685 8.77729,1.16744 14.01985,2.86167 27.41866,8.86073 26.43517,11.83601 27.15068,12.29991 41.25331,26.74942 8.70612,8.92023 14.09248,13.51982 15.8323,13.51982 2.85704,0 7.92419,-3.35522 31.17256,-20.64109 8.3323,-6.19537 15.64532,-11.26432 16.25094,-11.26432 2.44426,0 2.29271,4.33305 -0.80132,22.90645 -7.57207,45.45472 -16.39353,67.57866 -35.15064,88.1554 l -8.61832,9.45429 0.72342,53.08926 c 0.66257,48.61872 0.55098,53.23211 -1.32428,54.78554 -1.58064,1.30936 -7.25179,1.69625 -24.86394,1.69625 -12.54891,0 -24.33511,-0.17482 -26.19139,-0.38833 z m 36.65093,-215.6951 c -8.96927,-9.59127 -10.70895,-12.07192 -9.81702,-13.99868 0.59602,-1.28797 6.10537,-7.75225 12.24288,-14.36509 6.13752,-6.61283 13.49204,-14.72345 16.34316,-18.02362 2.85141,-3.30017 5.96911,-6.78507 6.92829,-7.74423 0.95918,-0.95915 7.31826,-8.01948 14.13134,-15.68959 6.81327,-7.67012 13.09672,-14.21781 13.96339,-14.55041 3.0536,-1.17177 31.35034,32.02737 31.35034,36.78181 0,1.70497 -3.92895,5.20257 -28.7675,25.6097 -10.67927,8.77397 -24.17532,19.95188 -29.99116,24.83974 -5.81574,4.88788 -11.6788,8.86684 -13.02862,8.84219 -1.48327,-0.0271 -6.76701,-4.65684 -13.3551,-11.70182 z m -40.5935,-25.7921 c -9.42413,-4.37803 -17.40624,-8.23152 -17.73803,-8.56333 -1.68457,-1.68457 4.56059,-10.91629 32.39718,-47.88996 34.3109,-45.573061 35.17193,-46.600646 37.79406,-45.100441 3.95518,2.262688 35.24152,33.466621 35.24152,35.148501 0,2.74056 -1.31091,4.3051 -45.46852,54.26449 -7.15859,8.09907 -14.0607,15.96805 -15.33806,17.48664 -3.59431,4.27292 -8.11961,3.3732 -26.88815,-5.3459 z m -52.81183,-20.27689 c -20.631648,-7.74926 -23.270868,-9.20701 -21.590488,-11.92496 0.83879,-1.35661 2.294888,-1.39005 8.095588,-0.18584 7.71003,1.6006 14.66268,1.05683 31.28831,-2.44709 22.60113,-4.76327 24.29029,-4.40538 20.17171,4.27384 -2.61893,5.51897 -14.10177,17.83297 -16.53583,17.73264 -0.98732,-0.0407 -10.63052,-3.39256 -21.42929,-7.44859 z m 50.32981,-43.26489 c 0.54018,-8.64524 0.40983,-14.80134 -0.34181,-16.14453 -0.66826,-1.19404 -7.49515,-5.259888 -15.17088,-9.035197 -8.16232,-4.014639 -14.10849,-7.693061 -14.32359,-8.860839 -0.35381,-1.921069 28.13524,-46.146427 30.9142,-47.990163 4.21681,-2.797638 7.37047,-0.635308 26.299,18.032371 l 19.2368,18.971536 -1.9444,3.475881 c -1.06942,1.911717 -8.80274,12.287692 -17.18526,23.057711 -8.3825,10.77004 -17.42282,22.48098 -20.08943,26.02431 -7.50244,9.96882 -8.42907,9.02505 -7.39463,-7.53108 z M 128.3971,80.359189 C 120.91066,77.845109 83.428802,61.190904 82.708172,60.058338 81.289002,57.827957 81.255012,57.890695 91.188342,44.418734 96.370562,37.390437 104.45921,26.394032 109.16323,19.982268 119.81555,5.4626375 123.3792,1.5752977 126.03748,1.5752977 c 2.45191,0 13.58055,7.9514137 27.87251,19.9148273 8.45645,7.078706 10.04484,8.964722 9.41042,11.173706 -0.99636,3.469398 -30.03286,47.199694 -31.75865,47.829972 -0.72812,0.265916 -2.15225,0.20534 -3.16466,-0.134614 z`
      );
  }

  private createMagicWandDef() {
    const magicWandDef = d3
      .select('svg')
      .append('defs')
      .append('g')
      .attr('id', 'magicWand')
      .attr('transform', 'scale(0.05 0.05)');

    magicWandDef.append('circle').attr('r', '500');

    magicWandDef
      .append('path')
      .attr('style', 'fill:#030104;')
      .attr('transform', 'translate(-220,-220)')
      .attr(
        'd',
        `M481.211,443.368L224.809,186.946l55.817-17.364c3.88-1.196,6.736-4.509,7.38-8.528c0.642-4.018-1.058-8.051-4.383-10.385  l-56.798-40.165c-9.814-6.913-15.56-18.2-15.417-30.172l0.916-69.519c0.051-4.06-2.23-7.796-5.837-9.647  c-3.608-1.844-7.976-1.483-11.225,0.962l-55.73,41.584c-9.588,7.163-22.117,9.136-33.439,5.294L40.246,26.678  c-3.848-1.315-8.118-0.322-11.007,2.55c-2.868,2.872-3.847,7.138-2.563,10.987l22.376,65.862c3.8,11.343,1.828,23.837-5.325,33.438  L2.131,195.245c-2.418,3.272-2.819,7.628-0.977,11.236c1.859,3.633,5.599,5.887,9.688,5.84l69.502-0.898  c11.971-0.153,23.244,5.582,30.174,15.366l40.149,56.807c2.373,3.32,6.401,5.021,10.412,4.387c4.025-0.632,7.314-3.497,8.518-7.378  l17.375-55.805l256.389,256.422c5.243,5.221,12.094,7.836,18.908,7.836c6.885,0,13.73-2.615,18.943-7.836  C491.671,470.772,491.671,453.826,481.211,443.368z`
      );
  }

  private createQuestionMarkDef() {
    const questionMarkDef = d3
      .select('svg')
      .append('defs')
      .append('g')
      .attr('id', 'questionMark')
      .attr('transform', 'scale(0.06 0.06)');

    questionMarkDef.append('circle').attr('r', '400');

    questionMarkDef
      .append('path')
      .attr('style', 'fill:#030104;')
      .attr('transform', 'translate(-250,-220)')
      .attr(
        'd',
        `M412.979,155.775C412.321,69.765,342.147,0,255.984,0c-86.57,0-157,70.43-157,157c0,11.046,8.954,20,20,20     s20-8.954,20-20c0-64.514,52.486-117,117-117s117,52.486,117,117c0,0.356,0.009,0.71,0.028,1.062     c-0.405,46.562-28.227,88.348-71.12,106.661c-40.038,17.094-65.908,56.675-65.908,100.839V412c0,11.046,8.954,20,20,20     c11.046,0,20-8.954,20-20v-46.438c0-28.117,16.334-53.258,41.614-64.051c57.979-24.754,95.433-81.479,95.418-144.516     C413.016,156.585,413.003,156.179,412.979,155.775z`
      );
  }

  private createFemaleDef() {
    const femaleDef = d3
      .select('svg')
      .append('defs')
      .append('g')
      .attr('id', 'female')
      .attr('transform', 'scale(0.08 0.08)');

    femaleDef.append('circle').attr('r', '300');

    femaleDef
      .append('path')
      .attr('style', 'fill:#030104;')
      .attr('transform', 'translate(-180,-180)')
      .attr(
        'd',
        `M284.17,89.848c-1.414-7.138-3.531-14.119-6.32-20.84c-2.796-6.751-6.253-13.209-10.32-19.28    c-8.179-12.246-18.685-22.765-30.92-30.96c-12.105-8.131-25.699-13.787-40-16.64c-14.422-2.837-29.258-2.837-43.68,0    c-14.301,2.852-27.896,8.508-40,16.64c-12.231,8.183-22.737,18.689-30.92,30.92c-8.131,12.105-13.786,25.699-16.64,40    c-2.838,14.422-2.838,29.258,0,43.68c2.852,14.301,8.508,27.896,16.64,40c10.815,16.169,25.646,29.25,43.04,37.96    c10.548,5.397,21.891,9.07,33.6,10.88c2.335,0.211,4.095,2.217,4,4.56v31.6c0,2.43-1.97,4.4-4.4,4.4h-46.64    c-3.148,0.013-6.165,1.263-8.4,3.48c-4.484,4.545-4.625,11.805-0.32,16.52l0.24,0.2c1.149,1.111,2.509,1.982,4,2.56    c1.433,0.596,2.968,0.909,4.52,0.92h46.72c1.169-0.005,2.291,0.456,3.12,1.28c0.761,0.828,1.176,1.916,1.16,3.04v46.76    c-0.006,1.563,0.293,3.112,0.88,4.56c0.572,1.494,1.444,2.855,2.56,4c1.145,1.117,2.506,1.989,4,2.56    c2.91,1.189,6.17,1.189,9.08,0c1.494-0.572,2.855-1.444,4-2.56c1.116-1.145,1.988-2.506,2.56-4c0.587-1.448,0.886-2.997,0.88-4.56    v-46.76c-0.005-1.169,0.456-2.291,1.28-3.12c0.829-0.824,1.951-1.285,3.12-1.28h46.68c1.563,0.006,3.112-0.293,4.56-0.88    c1.494-0.572,2.855-1.443,4-2.56c1.116-1.145,1.988-2.506,2.56-4c1.189-2.91,1.189-6.17,0-9.08    c-0.312-0.758-0.701-1.482-1.16-2.16l-1.56-1.56c-2.235-2.217-5.252-3.467-8.4-3.48h-46.76c-1.169,0.005-2.291-0.456-3.12-1.28    c-0.824-0.829-1.285-1.951-1.28-3.12v-29.44v-2.04c0.002-1.059,0.385-2.081,1.08-2.88c0.691-0.786,1.643-1.297,2.68-1.44    c11.874-1.678,23.397-5.272,34.12-10.64c24.149-12.068,43.127-32.451,53.44-57.4c2.791-6.734,4.908-13.728,6.32-20.88    C287.008,119.106,287.008,104.269,284.17,89.848z M260.85,128.848v0.16c-9.803,47.805-56.504,78.612-104.309,68.809    c-47.805-9.803-78.612-56.504-68.809-104.309c9.803-47.805,56.504-78.612,104.309-68.809    c41.276,8.464,70.833,44.895,70.609,87.029C262.612,117.479,262.009,123.214,260.85,128.848z`
      );
  }
  private createMaleDef() {
    const maleDef = d3
      .select('svg')
      .append('defs')
      .append('g')
      .attr('id', 'male')
      .attr('transform', 'scale(0.06 0.06)');

    maleDef.append('circle').attr('r', '400');

    maleDef
      .append('path')
      .attr('style', 'fill:#030104;')
      .attr('transform', 'translate(-200,-200)')
      .attr(
        'd',
        `m383.792969 13.9375c-.175781-1.378906-.480469-2.707031-.984375-3.953125-.015625-.03125-.015625-.074219-.023438-.113281 0-.007813-.007812-.015625-.015625-.023438-.554687-1.3125-1.3125-2.503906-2.167969-3.609375-.210937-.261719-.417968-.519531-.640624-.765625-.914063-1.03125-1.90625-1.984375-3.058594-2.761718-.03125-.023438-.070313-.03125-.101563-.054688-1.113281-.734375-2.34375-1.289062-3.632812-1.726562-.320313-.113282-.632813-.210938-.960938-.296876-1.351562-.367187-2.742187-.632812-4.207031-.632812h-112c-8.832031 0-16 7.167969-16 16s7.167969 16 16 16h73.367188l-95.496094 95.496094c-25.464844-20.367188-56.816406-31.496094-89.871094-31.496094-79.398438 0-144 64.601562-144 144s64.601562 144 144 144 144-64.601562 144-144c0-33.039062-11.121094-64.382812-31.503906-89.871094l95.503906-95.503906v73.375c0 8.832031 7.167969 16 16 16s16-7.167969 16-16v-112c0-.335938-.078125-.65625-.097656-.984375-.023438-.367187-.0625-.71875-.109375-1.078125zm-239.792969 338.0625c-61.761719 0-112-50.238281-112-112s50.238281-112 112-112c29.902344 0 58.054688 11.640625 79.222656 32.734375 21.136719 21.210937 32.777344 49.363281 32.777344 79.265625 0 61.761719-50.238281 112-112 112zm0 0`
      );
  }
}
